...
Make sure you have uploaded the `employee_handbook.pdf` into the container of the storage account before you start these steps.  The `emplopyee_handbook.pdf' can be found in the `Dotnet/src/BlazorAI/data` folder.

1. In Azure Portal, navigate to your AI Search Serivce -> Import and vectorize data

    ![AzureSearch](./Resources/images/ch05a01.png)

2. Select -> `Important and vectorize data` then select `Azure Blob Storage` and select the storage account and container and click next.

    ![AzureSearch2](./Resources/images/ch05a02-blob.png)

3. Select your Azure OpenAI service and embeddings model as well as API Key for Authenticaiton type and click `Next`.

    ![AzureSearch](./Resources/images/ch05a03-import.png)

4. On the `Vectorize and enrich your images' step, do not select anything here, simply click `Next`.

5. For the `Advance settings` step, check the `Enable semantic ranker` box and for `Schedule indexing` select once, then click 'Next'

    ![AzureSearch](./Resources/images/ch05a03-adv.png)

6. For the `Recview and create` step, set the `Object name prefix` to something meaningful, then click `Create`.

    ![AzureSearch](./Resources/images/ch05a03-create.png)

7. Once the index has been created you will be presented with `Created succeeded` dialog box, click on `Start searching` to test the index.  

    ![AzureSearch](./Resources/images/ch05a03-created.png)

8. Simply click on `Search` and you will be presented with the documents.  By default this will chuck the PDF into 1024 chunks as seen in the search tool.

    ![AzureSearch](./Resources/images/ch05a03-search.png)
...
2. Register the AI Search and the *Azure AI Search Vector Store connector* with the Kernel Builder. The connector is configured with the URL and API key of the Azure AI Search service.
...
4. The Sample RAG Plugin in the documentation maps the incoming data from AI Search to a class named ```IndexSchema```. Replace this class with the following code:
    ...
    :bulb: Note that the vector field to search is named ```text_vector``` but the sample code from the documentation uses ```vector```, **you will need to update the code to use the correct field name.**
...
5. Update the KernelFunction attribute to better help Semantic Kernel know when to call your function.

1. Add the plugin to Semantic Kernel using the ```AddFromType`` method.
